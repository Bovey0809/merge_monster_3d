...
class Center3DBoxCoder(PartialBinBasedBBoxCoder):
    ...
    def __init__(self,
        ...
        self.max_gt_num = 50
        self.grid_size = torch.round(((self.pc_range[3:] - self.pc_range[:3]) /
                                      self.voxel_size)).to(torch.long)
        self.output_size = (self.grid_size[:2] // downsample_ratio).to(
            torch.int)[[1, 0]]
    ...
    def generate_target_single(self, gt_labels_3d, gt_bboxes_3d):
        ...
        gt_scoremap = gt_labels_3d.new_zeros(
            (self.num_class, *self.output_size)).float()
        gt_corner_scoremap = gt_labels_3d.new_zeros(
            (self.num_class, *self.output_size)).float()
        ...
        boxes_dim2d_scaled = boxes_tensor[:, 3:
                                          5] / self.voxel_size[:2] / self.downsample_ratio
        boxes_center_2d = (boxes_tensor[:, 0:2] - self.pc_range[:2]
                           ) / self.voxel_size[:2] / self.downsample_ratio
        ...
        gt_corners_2d = (gt_corners_2d - self.pc_range[:2]
                         ) / self.voxel_size[:2] / self.downsample_ratio
        ...
        Center3DBoxCoder.generate_score_map(
            gt_corner_scoremap, gt_labels_3d_corner,
            boxes_dim2d_scaled.repeat_interleave(4, dim=0), gt_corners_int,
            self.min_overlap)

        gt_index[:num_boxes] = boxes_center_int[:, 1] * self.output_size[
            1] + boxes_center_int[:, 0]
        ...
        return (gt_scoremap, gt_xyz, gt_dim, gt_dir, gt_index, reg_mask,
                gt_corner_scoremap, gt_boxes3d)

    def generate_target(self, gt_labels_list, gt_bboxes_list):

        (all_gt_scoremap, all_gt_xyz, all_gt_dim, all_gt_dir, all_gt_index,
         all_reg_mask, all_gt_corner_scoremap,
         all_gt_boxes3d) = multi_apply(self.generate_target_single,
    ...
    def decode_center(self, pred_dict, img_metas, K=50, score_threshold=0.25):
        ...
        detections = []
        for clses, scores, bboxes, img_meta in zip(clses_batch, scores_batch,
                                                   bboxes_batch, img_metas):
    ...
    def decode_boxes(self, pred_dict):
        r"""
        decode output feature map to detection results
        """
        fmap = pred_dict['center_pred']
        ...
        bboxes_batch = bboxes_batch.permute(0, 2, 3,
                                            1).contiguous().reshape(-1, 7)
        boxes_pred_instances = LiDARInstance3DBoxes(
            bboxes_batch, origin=(0.5, 0.5, 0))
        corners_pred = boxes_pred_instances.corners.reshape(
        ...
        centers_pred[:, :, :, 0] = torch.clamp(centers_pred[:, :, :, 0], 0,
                                               width - 1)
        centers_pred[:, :, :, 1] = torch.clamp(centers_pred[:, :, :, 1], 0,
                                               height - 1)
        corners_pred[:, :, :, 0] = torch.clamp(corners_pred[:, :, :, 0], 0,
                                               width - 1)
        corners_pred[:, :, :, 1] = torch.clamp(corners_pred[:, :, :, 1], 0,
                                               height - 1)
    ...
    def generate_score_map(fmap, gt_class, gt_wh, centers_int, min_overlap):
        ...
        for i in range(gt_class.shape[0]):
            ...
            Center3DBoxCoder.draw_gaussian(fmap[channel_index], centers_int[i],
                                           radius[i].item())
    ...
    def draw_gaussian(fmap, center, radius, k=1):
        diameter = 2 * radius + 1
        gaussian = Center3DBoxCoder.gaussian2D((radius, radius),
                                               sigma=diameter / 6,
                                               rot=None)
