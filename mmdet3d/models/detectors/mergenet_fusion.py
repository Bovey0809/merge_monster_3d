...
class MergeNetFusion(Base3DDetector):
    ...
    def __init__(self,
        ...
        weight_parameters=torch.ones(1,requires_grad=True)*0.2
        self.weight_params=weight_parameters
    ...
    def extract_pts_feat(self, points):
        ...
        merge_fp_features = x['merge_fp_features'][-1]

        return (seed_points, seed_features, seed_indices, merge_fp_features)
    ...
    def merge_features(self, img_features, point_feat):
        ...
        if self.magic_merge=='imvotenet':
            point_feat=F.interpolate(
                point_feat, size=[400,352])

        if self.upsampe_style == 'interpolate':
            new_img_features = F.interpolate(
                img_features, size=[400,352])
