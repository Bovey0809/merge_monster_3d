...
class DiceLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        target = make_one_hot(target.unsqueeze(dim=1), classes=output.size()[1], clsoffset=self.clsoffset)
        output = F.softmax(output, dim=1)
        output_flat = output.contiguous().view(-1)
        target_flat = target.contiguous().view(-1)
        intersection = (output_flat * target_flat).sum()
        loss = 1 - (
            (2. * intersection + self.smooth) / (output_flat.sum() + target_flat.sum() + self.smooth)
        )
        return loss

class FocalLoss(nn.Module):
    def __init__(self, gamma=2, alpha=None, ignore_index=255, size_average=True, clsoffset=0):
...
class DepthwiseConv(nn.Module):
    def __init__(self, in_ch, out_ch):  
        ...
        self.depthwise = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1, groups=in_ch, bias=False), 
...
class SemanticHeadStuff(nn.Module):
    def __init__(self, in_ch32, in_ch64, in_ch128, hidden_ch, class_ts, droprate):
        ...
        self.predictor_thing_stuff = nn.Conv2d(hidden_ch, class_ts, kernel_size=1, stride=1, padding=0)
        self.predictor_thing_mask = nn.Conv2d(hidden_ch, 1, kernel_size=1, stride=1, padding=0)

        self.dice_loss_stuff = DiceLoss(smooth=1., ignore_index=255, clsoffset=80)
        ...
        self.focal_loss_stuff = FocalLoss(gamma=2, alpha=None, ignore_index=255, size_average=True, clsoffset=80)   
        self.focal_loss_thing = FocalLoss(gamma=2, alpha=None, ignore_index=255, size_average=True)      

        self.dice_loss_thing_mask = DiceLoss(smooth=1., ignore_index=255)
        self.focal_loss_thing_mask = FocalLoss_BCE(gamma=2, alpha=None, ignore_index=255, size_average=True) 
    ...
    def loss(self, preds, preds_thing_mask, gt): 
        ...
        preds_thing_mask = preds_thing_mask.permute(0, 2, 3, 1) # size = [b, h, w, c]
        # preds_thing_mask = torch.flatten(preds_thing_mask, 0, -2) # size = [bhw, c] 
        preds_thing_mask = torch.flatten(preds_thing_mask, 0, -1) # size = [bhw, c] 

        # dloss_thing_mask = self.dice_loss_thing_mask(preds_thing_mask, gt_mask_thing) 
        floss_thing_mask = self.focal_loss_thing_mask(preds_thing_mask, gt_mask_thing) 
