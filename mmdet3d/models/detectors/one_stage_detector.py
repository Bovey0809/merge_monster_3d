...
class OneStageDetector(nn.Module):
    def __init__(
        ...
        fpn_cfg=None,
        head_cfg=None,
        head_cfg_semantic_stuff=None,
    ):
        super(OneStageDetector, self).__init__()
        self.backbone = build_backbone(backbone_cfg)
        self.fpn = build_fpn(fpn_cfg)
        self.head = build_head(head_cfg)
        self.head_semantic_stuff = build_head(head_cfg_semantic_stuff)       

    def forward(self, x):
        x = self.backbone(x)
        feature128 = x[0]
        feature64 = x[1]
        x = self.fpn([x[1], x[2], x[3]]) # feature32
        x_box = self.head(x) 
        x_semantic_stuff, x_semantic_thing_mask = self.head_semantic_stuff(x[0], feature64, feature128)
    ...
    def inference(self, meta, class_names):
        with torch.no_grad():
            ...
            time1 = time.time()       
            preds_box, preds_semantic_stuff, preds_semantic_thing_mask = self(meta["img"])
            torch.cuda.synchronize()
            time2 = time.time()
            ...
            preds_semantic_stuff = self.head_semantic_stuff.post_process(preds_semantic_stuff, preds_semantic_thing_mask, meta)
    ...
    def forward_train(self, gt_meta):
        preds_box, preds_semantic_stuff, preds_semantic_thing_mask = self(gt_meta["img"])
        ...
        loss_semantic_stuff, loss_states_semantic_stuff = self.head_semantic_stuff.loss(
            preds_semantic_stuff, preds_semantic_thing_mask, gt_meta["img_semantic_stuff"]
        )
        ...
        return (preds_box, preds_semantic_stuff, preds_semantic_thing_mask), loss, loss_states
