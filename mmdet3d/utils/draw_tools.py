...
class Object3d(object):
    ...
    def __init__(self, label_file_line):
        ...
        try:
            ...
            self.score = 1.
        self.box3d = np.array([
            data[11], data[12], data[13], data[9], data[8], data[10], data[14]
        ]).astype(np.float32)
...
def draw_projected_boxes3d(image, boxes3d, color=(0, 255, 0), thickness=2):
    ...
    # print(image.shape)
    h, w, _ = image.shape
    # boxes3d=boxes3d.reshape(-1,2)
    # mask=np.any(boxes3d<-1000,axis=1)
    # boxes3d[mask]=np.abs(boxes3d[mask])
    # # boxes3d=np.abs(boxes3d)
    # # boxes3d[:,0]=np.clip(boxes3d[:,0],a_min=0,a_max=w)
    # # boxes3d[:,1]=np.clip(boxes3d[:,1],a_min=0,a_max=h)
    ...
    for qs in boxes3d:
        ...
        for k in range(0, 4):
            ...
            i, j = k, (k + 1) % 4
            # if np.any(qs[i]<0) or np.any(qs[j]<0):
            #     continue
...
def load_proposals(proosal_file, cls='Car'):
    ...
    for line in open(proosal_file, 'r'):
        ...
        if det_id2str[int(t[1])] == cls:
            if id in d.keys():
                d[id].append(
                    np.array([float(t[i]) for i in range(2, 7)],
                             dtype=np.float32))
            else:
                d[id] = [
                    np.array([float(t[i]) for i in range(2, 7)],
...
def draw_lidar(pc,
    ...
    if axis:
        # draw origin
        mlab.points3d(
            0, 0, 0, color=(1, 1, 1), mode='sphere', scale_factor=0.2)
        # draw axis
        axes = np.array([
            [2., 0., 0., 0.],
            [0., 2., 0., 0.],
